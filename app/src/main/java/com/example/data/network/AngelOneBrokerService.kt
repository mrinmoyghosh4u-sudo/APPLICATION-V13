...
class AngelOneBrokerService(
    ...
    override suspend fun placeOrder(order: OrderEntity): Result<String> {
        return runCatching {
            val token = if (order.symbolToken.isNotBlank()) order.symbolToken else com.example.util.InstrumentMapUtil.getAngelSymbolToken(order.symbol, order.exchange)
            ...
            if (response.isSuccessful && response.body()?.status == true) {
                response.body()?.data?.orderId ?: throw Exception("Order ID not returned by Angel One")
            } else {
                throw Exception(response.body()?.message ?: "Failed to place order on Angel One")
            }
        }
    }
    ...
    override suspend fun modifyOrder(orderId: String, newPrice: Double, newQty: Int, orderType: String): Result<Boolean> {
        return runCatching {
            ...
            if (response.isSuccessful && response.body()?.status == true) {
                ...
                throw Exception(response.body()?.message ?: "Failed to modify order on Angel One")
            }
        }
    }
    ...
    override suspend fun cancelOrder(orderId: String): Result<Boolean> {
        return runCatching {
            val response = api.cancelOrder(mapOf("orderid" to orderId))
            if (response.isSuccessful && response.body()?.status == true) {
                true
            } else {
                throw Exception(response.body()?.message ?: "Failed to cancel order on Angel One")
            }
        }
    }
    ...
    override suspend fun getMarketQuotes(symbols: List<String>): Result<List<WatchlistItem>> {
        return runCatching {
            ...
            symbols.forEach { sym ->
                val token = com.example.util.InstrumentMapUtil.getAngelSymbolToken(sym, "NSE")
                val ex = when {
            ...
            val response = runCatching { api.getQuotes(AngelQuoteRequest(exchangeTokens = tokenMap)) }.getOrNull()
            if (response != null && response.isSuccessful && response.body()?.status == true) {
