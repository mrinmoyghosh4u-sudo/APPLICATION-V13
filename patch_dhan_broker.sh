            ...
            if (response.isSuccessful) {
                ...
                items.map { item ->
                    ...
                    OrderEntity(
                        ...
                        productType = "INTRADAY", // Not natively in Order response from Dhan v2 easily without separate call
    ...
    override suspend fun placeOrder(order: OrderEntity): Result<String> {
        return runCatching {
            val req = DhanPlaceOrderRequest(
                ...
                exchangeSegment = if (order.exchange == "MCX") "MCX_COMM" else if (order.exchange == "BSE") "BSE_EQ" else "NSE_EQ",
        ...
        return runCatching {
            ...
            if (response.isSuccessful) {
                ...
                items.map { item ->
                    ...
                    val avgPrice = if (netQty > 0) item.buyAvg else if (netQty < 0) item.sellAvg else 0.0
                    val ltp = if (netQty > 0) {
                        item.buyAvg + (item.unrealizedProfit / netQty)
                    } else if (netQty < 0) {
                        item.sellAvg - (item.unrealizedProfit / abs(netQty))
                    } else {
                        0.0
                    }
                    ...
                    PortfolioHoldingEntity(
                        symbol = item.tradingSymbol,
                        exchange = item.exchangeSegment.ifBlank { "NSE" },
                        type = item.productType.ifBlank { "INTRADAY" },
                        qty = item.netQty,
                        avgPrice = avgPrice,
                        ltp = ltp,
